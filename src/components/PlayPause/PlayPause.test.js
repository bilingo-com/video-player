...
describe('PlayPause', () => {
  ...
  beforeAll(() => {
    component = shallow(
      <PlayPause ariaLabelPlay="Play" ariaLabelPause="Pause" />,
    );
  });
  ...
  describe('when paused', () => {
    ...
    it('shows a play icon', () => {
      expect(component.html()).toContain(PlayArrow);
      expect(component.html()).not.toContain(Pause);
    });

    it('has correct aria-label', () => {
      expect(component.find('button').prop('aria-label')).toEqual('Play');
    });
  });
  ...
  describe('when playing', () => {
    ...
    it('shows a pause icon', () => {
      expect(component.html()).toContain(Pause);
      expect(component.html()).not.toContain(PlayArrow);
    });

    it('has correct aria-label', () => {
      expect(component.find('button').prop('aria-label')).toEqual('Pause');
    });
  });
});
