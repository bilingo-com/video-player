...
describe('Seek', () => {
  ...
  it('has a range input with correct ranges and percentagePlayed value', () => {
    ...
    expect(rangeInput.prop('orient')).toEqual('horizontal');
    expect(rangeInput.prop('step')).toEqual(1);
    expect(rangeInput.prop('min')).toEqual('0');
    expect(rangeInput.prop('max')).toEqual('100');
    expect(rangeInput.prop('value')).toEqual(10);
  });
  ...
  it('changes the played fill bar width', () => {
    ...
    expect(component.find(`.${styles.fill}`).prop('style').width).toEqual('0%');
    component.setProps({
      percentagePlayed: 11,
    });
    expect(component.find(`.${styles.fill}`).prop('style').width).toEqual(
      '11%',
    );
  });
  ...
  it('changes the buffer bar width', () => {
    ...
    expect(component.find(`.${styles.buffer}`).prop('style').width).toEqual(
      '0%',
    );
    component.setProps({
