...
describe('Time', () => {
  ...
  it('shows video duration', () => {
    ...
    expect(component.find(`.${styles.duration}`).text()).toEqual('0:10');
  });
  ...
  it('shows current video elapsed time', () => {
    ...
    expect(component.find(`.${styles.current}`).text()).toEqual('0:10');
  });
  ...
  it('can handle minutes, hours and seconds', () => {
    ...
    expect(component.find(`.${styles.current}`).text()).toEqual('2:00');

    component.setProps({
      currentTime: 60 * 60 * 3,
    });
    expect(component.find(`.${styles.current}`).text()).toEqual('3:00:00');

    component.setProps({
      currentTime: 60 * 60 * 3 + 72,
    });
    expect(component.find(`.${styles.current}`).text()).toEqual('3:01:12');
  });
  ...
  it('fails gracefully and shows 00:00:00 when video is greater than 24 hours', () => {
    ...
    expect(component.find(`.${styles.current}`).text()).toEqual('0:00');
