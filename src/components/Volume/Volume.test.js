...
describe('Volume', () => {
  ...
  it('has a vertical range input with correct ranges and step', () => {
    ...
    expect(rangeInput.prop('type')).toEqual('range');
    expect(rangeInput.prop('orient')).toEqual('vertical');
    expect(rangeInput.prop('step')).toEqual(0.1);
    expect(rangeInput.prop('min')).toEqual('0');
    expect(rangeInput.prop('max')).toEqual('1');
    expect(rangeInput.prop('aria-label')).toEqual('Change volume');
  });
  ...
  it('handles an undefined volume value', () => {
    ...
    const rangeInput = component.find(`.${styles.input}`);
    expect(rangeInput.prop('value')).toEqual(0);
  });
  ...
  describe('when muted', () => {
    ...
    it('shows a muted icon', () => {
      expect(component.html()).toContain(VolumeOff);
      expect(component.html()).not.toContain(VolumeUp);
    });

    it('has an empty track fill and range input', () => {
      expect(component.find(`.${styles.fill}`).prop('style').height).toEqual(
        '0%',
      );
      expect(component.find(`.${styles.input}`).prop('value')).toEqual(0);
    });

    it('has correct aria-label', () => {
      expect(component.find('button').prop('aria-label')).toEqual('Unmute');
    });
  });
  ...
  describe('when has volume and is not muted', () => {
    ...
    it('shows an unmute icon', () => {
      expect(component.html()).toContain(VolumeUp);
      expect(component.html()).not.toContain(VolumeOff);
    });
    ...
    it('has some track filled and a range input value', () => {
      ...
      expect(component.find(`.${styles.input}`).prop('value')).toEqual(0.5);
    });

    it('has correct aria-label', () => {
      expect(component.find('button').prop('aria-label')).toEqual('Mute');
    });
  });
});
