...
describe('DefaultPlayer', () => {
  ...
  it('applies `style` prop if provided', () => {
    ...
    expect(component.prop('style')).toEqual({ color: 'red' });
  });
  ...
  it('spreads all parent props on the video element', () => {
    ...
    expect(component.find('video').prop('autoPlay')).toEqual(true);
  });
  ...
  it('renders some default controls in a default order', () => {
    ...
    expect(controlsComponent.childAt(0).is(PlayPause)).toBeTruthy();
    expect(controlsComponent.childAt(1).is(Seek)).toBeTruthy();
    expect(controlsComponent.childAt(2).is(Time)).toBeTruthy();
    expect(controlsComponent.childAt(3).is(Volume)).toBeTruthy();
    expect(controlsComponent.childAt(4).is(Fullscreen)).toBeTruthy();
  });
  ...
  it('renders controls in a given custom order', () => {
    ...
    const controlsComponent = component.find(`.${styles.controls}`);
    expect(controlsComponent.childAt(0).is(Fullscreen)).toBeTruthy();
    expect(controlsComponent.childAt(1).is(Seek)).toBeTruthy();
    expect(controlsComponent.childAt(2).is(Time)).toBeTruthy();
  });

  it('renders no controls when given an empty array', () => {
    expect(component.find(`.${styles.controls}`).exists()).toBeTruthy();
    component.setProps({
      controls: [],
    });
    expect(component.find(`.${styles.controls}`).exists()).toBeFalsy();
  });
